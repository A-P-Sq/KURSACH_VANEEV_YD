        ...
        private void BtnExportCars_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SaveFileDialog sfd = new SaveFileDialog()
                {
                    ...
                    FileName = $"Cars_Report_{DateTime.Now:dd/MM/yy}.csv"
                };
                ...
                if (sfd.ShowDialog() == true)
                {
                    ...
                    // Заголовки (с точкой с запятой для корректного открытия в Excel)
                    csv.AppendLine("ID;Марка;Модель;Год;Цена;Статус");

                    foreach (DataRowView row in view)
                    {
                        string soldStatus = (bool)row["is_sold"] ? "Продано" : "В продаже";
                        csv.AppendLine($"{row["id"]};{row["brand"]};{row["model"]};{row["year"]};{row["price"]};{soldStatus}");
                    }

                    // Сохраняем в кодировке UTF-8 с BOM (чтобы Excel понимал кириллицу)
